...
def cross_validate_estimators() -> None:
    print(
        f"{'Alpha':<10} | {'Exact':<10} | {'Approx':<10} | {'Error (%)':<10} | {'Exact Time':<10} | {'Approx Time':<10}"
    )
    print("-" * 75)
    ...
    for alpha in alphas:
        ...
        print(
            f"{alpha:<10.2f} | {h_exact.item():<10.4f} | {h_approx.item():<10.4f} | {error.item():<10.2f}% | {exact_time:<10.4f} | {approx_time:<10.4f}"
        )
