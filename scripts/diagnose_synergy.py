...
def diagnose() -> None:
    ...
    # Check PID at Step 1
    pid_1 = estimate_pid_renyi(
        out_1["agg_ff_signal"], out_1["agg_ctx_signal"], out_1["final_rep"]
    )
    print("\n--- Step 1 PID ---")
    ...
    for epoch in range(100):
        ...
        if (epoch + 1) % 20 == 0:
            pid = estimate_pid_renyi(
                out_1["agg_ff_signal"], out_1["agg_ctx_signal"], out_1["final_rep"]
            )
            print(
                f"Epoch {epoch+1:03d} | Loss: {loss.item():.4f} | Synergy: {pid['synergy'].item():.4f} | Redundancy: {pid['redundancy'].item():.4f}"
            )
